...
GridDB deliberately narrowed to two, because DEVICE_BASE and SYSTEM_BASE differ only in WHICH
NUMBER a pu value is normalized against, not in how the number is interpreted -- and GridDB
rows now carry or can reach that number themselves (base_power/base_voltage columns, or
base_power_ref/base_voltage_ref FK paths), so the upstream label is redundant here. L1 maps
schema discriminator values through this table before comparing key sets/units, so the
narrowing is recognized as consistent rather than reported as a contradiction:
    ...
    schema DEVICE_MVAR   -> GridDB NATURAL_UNITS, disambiguated by quantity_type
...
# DEVICE_BASE and SYSTEM_BASE both collapse onto COMPONENT_BASE (see module docstring); a key
# absent from this map (e.g. AC_VOLTAGE, DC_POWER, MWH -- a different discriminator entirely)
# passes through unchanged, so unrelated discriminators are never touched by this mapping.
BASIS_ALIAS = {
    "DEVICE_BASE": "COMPONENT_BASE",
...
BASIS_VOCAB = frozenset({"DEVICE_BASE", "SYSTEM_BASE", "NATURAL_UNITS"})
    ...
    for (table, column), rows in group_conventions(conventions):
        ...
        for entry in tables[table]:
            ...
            if ann["unit"] is None and _is_flat_basis_map(ann["units_map"]):
                ...
                # device base, always COMPONENT_BASE, never NATURAL_UNITS). Compare against
...
def _is_flat_basis_map(units_map):
    ...
    Distinguishes a basis discriminator (DEVICE_BASE/SYSTEM_BASE/NATURAL_UNITS) from an
...
def _collapse_basis_keys(expanded_map):
    """Map DEVICE_BASE/SYSTEM_BASE onto COMPONENT_BASE in each (primary, secondary) key of an
    ...
    mean DEVICE_BASE and SYSTEM_BASE were annotated with different units for the same column --
