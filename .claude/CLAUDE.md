...
- No converter loads a PSY `System` into these tables or back. Table shapes and `column_conventions.json` deliberately mirror PSY fields (natural units; cost curves in either basis, keyed on their own `power_units`; `base_power` per-unitization columns) so the future bridge is mechanical.
